...
public class Warning {
    ...
    @CommandPermissions({"punishment.staff.warn"})
    public static void warn(CommandContext args, CommandSender sender) {
        if (sender instanceof Player) {
            ...
            if (target == null) {
                MessageUtils.messagePrefix(player, "Can't find player &6" + args.getString(0) + "&7.");
                return;
            } else if (Bukkit.getPlayer(target).hasPermission("punishment.staff")) {
                BungeeUtils.warnStaff(player, NameUtility.getName(target));
                MessageUtils.messagePrefix(player, "&c&lYou may not warn other staff! All staff online have been notified and this attempt has been logged.");
                return;
            }
            ...
            if (targetPlayer != null) {
                ...
                MessageUtils.messagePrefix(player, "Warned &6" + args.getString(0) + "&7.");
            } else {
                MessageUtils.messagePrefix(player, "&cYou can't warn an offline player! (They must be on &6&lYOUR &cserver.)");
            }
        }
    }
}
